...
(function () {
    ...
    function renderLeaderboard() {
        ...
        container.innerHTML = '';
        if (!lb.length) {
            container.innerHTML = '<p class="leaderboard-empty">No scores yet. Mainkan untuk mengisi leaderboard!</p>';
            return;
        }
        ...
        lb.forEach(item => {
            ...
            li.innerHTML = `<span class="lb-name">${escapeHtml(item.name)}</span><span class="lb-score">${item.score}</span>`;
            list.appendChild(li);
        });
    ...
    function seedLeaderboardIfEmpty() {
        ...
        const now = new Date();
        const sampleEntries = [
            { name: 'Champion', score: 999999, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 1).toISOString() }, // tertinggi
            { name: 'ProGamer', score: 850000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 2).toISOString() },
            { name: 'Ace', score: 650000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 3).toISOString() },
            { name: 'TopPlayer', score: 500000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 4).toISOString() },
            { name: 'Legend', score: 420000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 5).toISOString() },
            { name: 'Player1', score: 300000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 6).toISOString() },
            { name: 'Player2', score: 200000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 7).toISOString() },
            { name: 'Player3', score: 150000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 8).toISOString() },
            { name: 'Player4', score: 120000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 9).toISOString() },
            { name: 'Player5', score: 100000, date: new Date(now.getTime() - 1000 * 60 * 60 * 24 * 10).toISOString() }
        ];

        // Ensure sorted and top 10 only
        sampleEntries.sort((a, b) => b.score - a.score || new Date(b.date) - new Date(a.date));
        localStorage.setItem(LS_LEADERBOARD_KEY, JSON.stringify(sampleEntries.slice(0, 10)));
    }
